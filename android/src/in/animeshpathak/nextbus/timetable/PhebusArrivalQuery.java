...
public class PhebusArrivalQuery extends BusArrivalQuery {
	...
	private static final String[] MIN =		{"minute1","minute2","minute3","minute4"};
	private static final String[] STATIC =	{"static1","static2","static3","static4"};
	 ...
	 * Unfortunately capture-groups cannot be repetitive in a regular expression.
	 * This is why the capture-group names must be unique. 
	 * 
	 * @param queryRes
	 * @return
	 */
	...
	private BusArrivalInfo createBusArrivalInfo(Map<String, String> groups, int arrivals, String dir1, 
		...
		if(groups.get(min1)!=null || groups.get(static1)!=null || groups.get(app1) != null){
			b1Mentions[0] |= (groups.get(star1) != null)?BusArrivalInfo.MENTION_THEORETICAL:0;
			b1Mentions[0] |= (groups.get(last1) != null)?BusArrivalInfo.MENTION_LAST:0;
			if((groups.get(app1) != null)){
				b1Mentions[0] |= (groups.get(app1) != null)?BusArrivalInfo.MENTION_APPROACHING:0;
				b1Arrivals[0] = 0;
			}
	...
	/**
	 * In the Phebus service, the time is either represented in minutes (relative to bus arrival) e.g., "14 min",
	 * or in an absolute "Time-table" fashion, e.g., "9h31"
	 * 
	 * This method will transform the "time-table" format into a relative "arrival time"
	 * @return
	 * @throws ParseException 
	 */
	private int frenchHourToArrivalMillis(String st) throws ParseException{
		...
		foundTime.set(	currentTime.get(Calendar.YEAR), 
